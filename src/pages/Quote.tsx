          ...
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
              <div className="p-6 lg:p-8 rounded-xl bg-white border border-[#00000008] shadow-sm">
                <h2 className="font-heading text-xl font-bold text-[#1D0E03] mb-6">Project Details</h2>
                {done ? (
                  <div className="text-center py-12">
                    <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4" />
                    <h3 className="font-heading text-xl font-bold text-[#1D0E03] mb-2">Quote Request Received</h3>
                    <p className="text-[#56483E] mb-6">Thank you! We'll review your project details and get back to you within 24 hours.</p>
                  ...
                  <form onSubmit={submit} className="space-y-5">
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <label htmlFor="q-name" className="block text-sm font-heading font-medium text-[#1D0E03] mb-2">Full Name <span className="text-[#CA5502]">*</span></label>
                        <input id="q-name" value={form.name} onChange={(e) => set('name', e.target.value)}
                          className={`w-full px-4 py-3 rounded-lg bg-[#FFF6EF] border text-[#1D0E03] text-sm placeholder-[#8E8279] focus:outline-none focus:ring-2 focus:ring-[#CA5502]/50 focus:border-[#CA5502] transition-colors ${errs.name ? 'border-red-500' : 'border-[#00000012]'}`} placeholder="John Smith" />
                        {errs.name && <p className="mt-1 text-xs text-red-500">{errs.name}</p>}
                      </div>
                      <div>
                        <label htmlFor="q-email" className="block text-sm font-heading font-medium text-[#1D0E03] mb-2">Email <span className="text-[#CA5502]">*</span></label>
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <label htmlFor="q-phone" className="block text-sm font-heading font-medium text-[#1D0E03] mb-2">Phone</label>
                      ...
                      <div>
                        <label htmlFor="q-company" className="block text-sm font-heading font-medium text-[#1D0E03] mb-2">Company</label>
                        <input id="q-company" value={form.company} onChange={(e) => set('company', e.target.value)}
                          className="w-full px-4 py-3 rounded-lg bg-[#FFF6EF] border border-[#00000012] text-[#1D0E03] text-sm placeholder-[#8E8279] focus:outline-none focus:ring-2 focus:ring-[#CA5502]/50 focus:border-[#CA5502] transition-colors" placeholder="Company name" />
                      </div>
                    </div>
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <label htmlFor="q-service" className="block text-sm font-heading font-medium text-[#1D0E03] mb-2">Service Needed</label>
                        <select id="q-service" value={form.service} onChange={(e) => set('service', e.target.value)}
                          className="w-full px-4 py-3 rounded-lg bg-[#FFF6EF] border border-[#00000012] text-[#1D0E03] text-sm focus:outline-none focus:ring-2 focus:ring-[#CA5502]/50 focus:border-[#CA5502] transition-colors">
                          <option value="">Select a service</option>
                          {services.map((s) => <option key={s.id} value={s.slug}>{s.title}</option>)}
                        </select>
                      </div>
                      <div>
                        <label htmlFor="q-timeline" className="block text-sm font-heading font-medium text-[#1D0E03] mb-2">Timeline</label>
                    ...
                    <div>
                      ...
                      <textarea id="q-desc" rows={5} value={form.description} onChange={(e) => set('description', e.target.value)}
                        className={`w-full px-4 py-3 rounded-lg bg-[#FFF6EF] border text-[#1D0E03] text-sm placeholder-[#8E8279] focus:outline-none focus:ring-2 focus:ring-[#CA5502]/50 focus:border-[#CA5502] transition-colors resize-none ${errs.description ? 'border-red-500' : 'border-[#00000012]'}`} placeholder="Tell us about your project..." />
            ...
            <div>
              <div className="sticky top-24 space-y-5">
                ...
                <div className="p-5 rounded-xl bg-[#FFF6EF] border border-[#00000008] text-center shadow-sm">
                  <p className="text-sm text-[#56483E] mb-2">Prefer to call?</p>
